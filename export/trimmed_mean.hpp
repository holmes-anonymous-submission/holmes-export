...
void finalize_trimmed_mean(uint64_t effective_sum, IntFp zk_effective_sum, int reference_n, uint64_t effective_count, IntFp zk_effective_count, uint64_t &trimming_mean, IntFp &zk_trimming_mean, int fixed_point_shift, vector<IntFp> &zk_zero_checking) {
	// Store mean in fixed-point
	trimming_mean = effective_sum * fixed_point_shift / effective_count;
	zk_trimming_mean = IntFp(trimming_mean, ALICE);

	// Check that: #subsamples * mean_shifted <= sum * fixed_point_shift < #subsamples * mean_shifted_plus_one

	uint64_t gap = effective_sum * fixed_point_shift - trimming_mean * effective_count;
	IntFp zk_gap = zk_effective_sum * fixed_point_shift + zk_trimming_mean * zk_effective_count.negate();

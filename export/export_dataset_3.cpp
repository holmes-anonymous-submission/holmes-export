...
int main(int argc, char** argv) {
	...
	uint64_t total_impressions_trimming_mean = 0;
	IntFp zk_total_impressions_trimming_mean = IntFp(0, PUBLIC);

	finalize_trimmed_mean(total_impressions_trimming_sum, zk_total_impressions_trimming_sum, dataset.size(), total_impressions_trimming_count, zk_total_impressions_trimming_count, total_impressions_trimming_mean, zk_total_impressions_trimming_mean,  100, zk_zero_checking);

	if(party == ALICE) {
		cout << "the trimmed mean for total impressions = " << total_impressions_trimming_mean * 0.01 << endl;
	}
	...
	// mean and variances
	precomputed_dataset.push_back(total_impressions_trimming_mean);
	zk_precomputed_dataset.push_back(zk_total_impressions_trimming_mean);
	...
	#ifdef ARGUMENT_EXPORT
	ZKFpExec::zk_exec->end_argument_exporter(1000);
	#endif
